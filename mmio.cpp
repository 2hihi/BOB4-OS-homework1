...
bool read_file_using_memory_map()
{
	...
	LPCWSTR file_name2 = L"C:\\Users\\Hestia\\Documents\\Visual Studio 2013\\BOB\\OS\\hw1_MemoryMappedIO\\hw1_MemoryMappedIO\\bob2.txt";
	...
	// UTF8 -> unicode -> multibyte
	int len = MultiByteToWideChar(CP_UTF8, 0, file_view, strlen(file_view), NULL, NULL); // file_view <- utf8
	wchar_t pMultiByte[256] = { 0, };
	MultiByteToWideChar(CP_UTF8, 0, file_view, strlen(file_view), pMultiByte, len); // file_view(utf8) -> pMultibyte(unicode)
